...
class ImageChartsTest {

    private static final String CHART_DATA = "t:1,2,3";
    private static final String CHART_SIZE = "100x100";
    private static final String CHS_MESSAGE = "\"\\\"chs\\\" is required\"";
    ...
    void toUrlWorks()
        ...
        String url = new ImageCharts().cht("p").chd(CHART_DATA).toURL();
    ...
    void toUrlAddSignature()
            throws MalformedURLException, NoSuchAlgorithmException, InvalidKeyException,
                    UnsupportedEncodingException {
        String url =
                new ImageCharts("plop")
                        .cht("p")
                        .chd(CHART_DATA)
                        .chs(CHART_SIZE)
                        .icac("test_fixture")
    ...
    void toUrlExposesParametersUseThem()
    ...
    void toBufferRejectsIfChsNotDefined() {
        ImageChartsException exception =
                assertThrows(
                        ImageChartsException.class,
                        new Executable() {
                            @Override
                            public void execute() throws Throwable {
                                new ImageCharts().cht("p").chd(CHART_DATA).toBuffer();
                            }
                        });

        String expectedMessage = CHS_MESSAGE;
    ...
    void toBufferRejectsIfIcacWithoutIchm() {
        ...
        String expectedMessage =
                "\"The `icac` (ACCOUNT_ID) and `ichm` (HMAC-SHA256 request signature) query"
                        + " parameters must both be defined if specified. [Learn"
                        + " more](https://bit.ly/HMACENT)\"";
    ...
    void toBufferWorks() throws NoSuchAlgorithmException, InvalidKeyException, IOException {
        new ImageCharts().cht("p").chd(CHART_DATA).chs(CHART_SIZE).toBuffer();
    }
        ...
        ImageChartsException exception =
                assertThrows(
                        ...
                        new Executable() {
                            @Override
                            public void execute() throws Throwable {
                                new ImageCharts().cht("p").chd(CHART_DATA).toDataURI();
                            }
                        });
    ...
    void toDataURIWorks() throws NoSuchAlgorithmException, InvalidKeyException, IOException {
        String dataURI = new ImageCharts().cht("p").chd(CHART_DATA).chs("2x2").toDataURI();

        assertEquals(
                "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAIAAAACAQAAAABazTCJAAAADElEQVR4XmM4wHAAAAMEAYHt3ShHAAAAAElFTkSuQmCC",
                dataURI);
    }
    ...
    void toDataURISupportGifs() throws NoSuchAlgorithmException, InvalidKeyException, IOException {
        String dataURI =
                new ImageCharts().cht("p").chd(CHART_DATA).chan("100").chs("2x2").toDataURI();
        ...
        ImageChartsException exception =
                assertThrows(
                        ...
                        new Executable() {
                            @Override
                            public void execute() throws Throwable {
                                new ImageCharts().cht("p").chd(CHART_DATA).toFile("/tmp/chart.png");
                            }
                        });
                        ...
                        new Executable() {
                            ...
                            public void execute() throws Throwable {
                                new ImageCharts()
                                        .cht("p")
                                        .chd(CHART_DATA)
                                        .toFile("/__invalid_path/chart.png");
                            }
                        });
    ...
    void toFileWorks() throws NoSuchAlgorithmException, InvalidKeyException, IOException {
        ...
        String filePath = "/tmp/plop.png";
        new ImageCharts().cht("p").chd(CHART_DATA).chan("100").chs("2x2").toFile(filePath);
    ...
    void hostUserDefined() throws NoSuchFieldException, IllegalAccessException {
        ImageCharts builder =
                new ImageCharts(null, "on-premise-image-charts.com", null, null, null, null);
    ...
    void queryUserDefined() throws NoSuchFieldException, IllegalAccessException {
        ImageCharts builder = new ImageCharts().cht("p").chd(CHART_DATA).icac("plop");

        Field protocolField = builder.getClass().getDeclaredField("query");
        protocolField.setAccessible(true);
        Map<String, Object> query = (Map<String, Object>) protocolField.get(builder);
        assertEquals("p", query.get("cht"));
        assertEquals(CHART_DATA, query.get("chd"));
